...
class GlobalFeatureMetric:
    ...
    def gradual_perturbation(self, model, X, y,  importances_orig, column_transformer, preprocessing_pipeline=None,
        ...
        intermediate_importances = self.create_intermediate_points(np.zeros(len(inv_norm_importances)),
    ...
    def gradual_perturbation_step(self,model, X, y, baseline_accuracy,  importances, column_transformer, preprocessing_pipeline=None,
                                  count_per_step=5):
    ...
    def create_intermediate_points(self, start_vals, end_vals, resolution):
        arr = []
        for start_val, end_val in zip(start_vals, end_vals):
            arr.append(np.linspace(start_val, end_val, resolution))
        return np.array(arr).T

    # def parallel_task(dataset, importances, n_jobs, task_wrapper, params=()):
    #     from multiprocessing import Pool, cpu_count
    #     with Pool(cpu_count()) as p:
    #         ret_list = p.starmap(task_wrapper, [(dataset)
    #         p.close()
